...
#define SYNUSB_RECV_SIZE	55
#define SYNUSB_CTRL_RECV_SIZE	12
...
struct synusb {
	...
	struct urb *control_urb2;
	unsigned char *data;
	unsigned char *control_data;
	unsigned char *control_data2;
...
static void synusb_report_touchpad(struct synusb *synusb)
{
	...
	printk("synusb_report_touchpad i:%i %x %x %x %x %x %x %x %x",
			synusb->intf->cur_altsetting->desc.bInterfaceNumber,
			synusb->data[0],
			synusb->data[1],
			synusb->data[2],
			synusb->data[3],
...
synusb_get_in_endpoint(struct usb_host_interface *iface)
{
	...
	printk("synusb_get_in_endpoint bNumEndpoints:%i", iface->desc.bNumEndpoints);
	for (i = 0; i < iface->desc.bNumEndpoints; ++i) {
		endpoint = &iface->endpoint[i].desc;
		printk("synusb_get_in_endpoint endpoint:%x", endpoint->bEndpointAddress);
...
static void synusb_ctrl_irq(struct urb *urb)
{
	struct synusb *synusb = urb->context;
	int error;

	printk("synusb_ctrl_irq intf:%i stage:%i status:%i", synusb->intf->cur_altsetting->desc.bInterfaceNumber, synusb->init_stage, urb->status);
	synusb->msg_count++;
	if (synusb->init_stage == 1) {
		synusb->msg_count++;
		printk("stream_id: %i %i", urb->stream_id, synusb->msg_count);
		if (synusb->msg_count >= 3) {
			...
			if (!dr)
				return dev_err(&synusb->intf->dev, "kmalloc fails");

			dr->bRequestType = USB_DT_WIRE_ADAPTER;
			dr->bRequest = USB_REQ_SET_CONFIGURATION;
			dr->wValue = cpu_to_le16(0x0201);
			dr->wIndex = cpu_to_le16(0);
			dr->wLength = cpu_to_le16(2);

			printk("send my control 1");
			usb_fill_control_urb(synusb->control_urb, synusb->udev,
					 usb_sndctrlpipe(synusb->udev, 0),
					 (unsigned char *)dr,
					 synusb->control_data, 2,
					 synusb_ctrl_irq, synusb);
			//synusb->control_urb->transfer_dma = dev->ctl_dma;
			synusb->control_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
			synusb->control_urb->dev = synusb->udev;
			
			error = usb_submit_urb(synusb->control_urb, GFP_ATOMIC);
			if (error)
			...
			printk("data %02hhX %02hhX", synusb->control_data[0], synusb->control_data[1]);
			...
			dr->bRequestType = USB_DT_WIRE_ADAPTER;
			dr->bRequest = USB_REQ_GET_INTERFACE;
			dr->wValue = cpu_to_le16(0x0000);
			dr->wIndex = cpu_to_le16(2 - synusb->msg_count);
	...
	if (synusb->init_stage == 2) {
		...
		dr->bRequestType = USB_DT_WIRE_ADAPTER;
		dr->bRequest = USB_REQ_SET_CONFIGURATION;
		dr->wValue = cpu_to_le16(0x030c);
		dr->wIndex = cpu_to_le16(1);
		dr->wLength = cpu_to_le16(3);

		printk("send my control 2");
		usb_fill_control_urb(synusb->control_urb, synusb->udev,
				 usb_sndctrlpipe(synusb->udev, 0),
				 (unsigned char *)dr,
				 synusb->control_data, 2,
	...
	if (synusb->init_stage == 3) {
		struct usb_ctrlrequest *dr;

		synusb->init_stage = 4;
		synusb->msg_count = 0;
		printk("data %02hhX %02hhX %02hhX", synusb->control_data[0], synusb->control_data[1], synusb->control_data[2]);
		...
		dr->wValue = cpu_to_le16(0x030b);
		dr->wIndex = cpu_to_le16(1);
		dr->wLength = cpu_to_le16(12);

		printk("send my control 3");
		usb_fill_control_urb(synusb->control_urb2, synusb->udev,
				 usb_rcvctrlpipe(synusb->udev, 0x80),
				 (unsigned char *)dr,
				 synusb->control_data2, 12,
				 synusb_ctrl_irq, synusb);
		synusb->control_urb2->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
		synusb->control_urb2->dev = synusb->udev;
		
		error = usb_submit_urb(synusb->control_urb2, GFP_ATOMIC);
		if (error)
			dev_err(&synusb->intf->dev,
				"%s - usb_submit_urb failed with result: %d",
				__func__, error);
	...
	if (synusb->init_stage == 4) {
		struct usb_endpoint_descriptor *ep;
		synusb->init_stage = -1;
		printk("data %02hhX %02hhX %02hhX", synusb->control_data2[0], synusb->control_data2[1], synusb->control_data2[2]);
		/*usb_fill_int_urb(synusb->urb, synusb->udev,
			 usb_rcvintpipe(synusb->udev, 0x83),
			 synusb->data, SYNUSB_RECV_SIZE,
			 synusb_irq, synusb,
			 2);
		synusb->urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;*/
		synusb_open(synusb->input);
		/*ep = synusb_get_in_endpoint(synusb->intf->cur_altsetting);

		usb_fill_int_urb(synusb->urb, synusb->udev,
			 usb_rcvintpipe(synusb->udev, ep->bEndpointAddress),
			 synusb->data, SYNUSB_RECV_SIZE,
			 synusb_irq, synusb,
			 ep->bInterval);
		synusb->urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
...
static void synusb_irq(struct urb *urb)
{
	...
	printk("synusb_irq intf:%i stage:%i status:%i", synusb->intf->cur_altsetting->desc.bInterfaceNumber, synusb->init_stage, urb->status);

	if (urb->status == -2) {
		printk("unknow init stage and option URB");
		return;
	} else
...
static int synusb_open(struct input_dev *dev)
{
	...
	printk("synusb_open retval:%i", retval);
			...
			const struct usb_device_id *id)
{
	...
	printk("synusb_probe alt:%i alt_num:%i", intf_num, intf->num_altsetting);
	//if (intf_num != 1) {
	//	synusb->init_stage = -1;
	//	return -ENODEV;
	//}
	...
	synusb->control_urb = usb_alloc_urb(0, GFP_KERNEL);
	if (!synusb->control_urb) {
		error = -ENOMEM;
		goto err_free_mem;
	}

	synusb->control_urb2 = usb_alloc_urb(0, GFP_KERNEL);
	if (!synusb->control_urb) {
		error = -ENOMEM;
		goto err_free_mem;
	}
	...
	synusb->control_data = usb_alloc_coherent(udev, SYNUSB_CTRL_RECV_SIZE, GFP_KERNEL,
					  &synusb->control_urb->transfer_dma);
	if (!synusb->control_data) {
		error = -ENOMEM;
		goto err_free_urb;
	}

	synusb->control_data2 = usb_alloc_coherent(udev, SYNUSB_CTRL_RECV_SIZE, GFP_KERNEL,
					  &synusb->control_urb2->transfer_dma);
	if (!synusb->control_data) {
		error = -ENOMEM;
		goto err_free_urb;
	}
	...
	printk("bInterval:%i", ep->bInterval);
	usb_fill_int_urb(synusb->urb, udev,
			 usb_rcvintpipe(udev, 0x83),
			 synusb->data, SYNUSB_RECV_SIZE,
			 synusb_irq, synusb,
			 2);
	synusb->urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
...

	{
		...
		if (!dr) {
			dev_err(&synusb->intf->dev, "kmalloc fails");
			return -EIO;
		}
		...
		dr->bRequest = 10;
		dr->wValue = cpu_to_le16(0x0000);
		dr->wIndex = cpu_to_le16(2);
		dr->wLength = cpu_to_le16(0);

		printk("synusb_open send first init");

		usb_fill_control_urb(synusb->control_urb, synusb->udev,
				 usb_sndctrlpipe(synusb->udev, 0),
				 (unsigned char *)dr,
				 synusb->control_data, 0,
				 synusb_ctrl_irq, synusb);
		synusb->control_urb->transfer_flags |= URB_NO_TRANSFER_DMA_MAP;
		synusb->control_urb->dev = synusb->udev;
...
static struct usb_device_id synusb_idtable[] = {
	{ 
		USB_DEVICE_INTERFACE_NUMBER(0x0b05, 0x1823, 0),
		.driver_info = (SYNUSB_TOUCHPAD),
	},
